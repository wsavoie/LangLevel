...
def mecab():  # IO MecabProc
    ...
    try:
        print('found mecab Unidic')
        reading = importlib.import_module('deps.mecabUnidic.reading')
    ...
    if not reading:
        try:
            reading = importlib.import_module('deps.mecab.reading')
            mecab_source = 'ur mom'
            print('using oldest mecab')
        except ModuleNotFoundError:
            pass
    # 7th priority - system mecab
    if not reading:
        try:
